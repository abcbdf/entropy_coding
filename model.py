...
class Model_Entropy(nn.Module):
    ...
    def forward(self, input):
        ...
        output = Variable(torch.zeros(batch, channel, height, width))
        for iter in range(height * width):
            p = F.sigmoid(self.conv(self.lstm(self.masked_conv(new_input, iter), iter)))
            h = int(iter / width)
            w = iter - h * width
            output[:, :, h, w] = p
        return output
